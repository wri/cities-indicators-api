...
def list_cities(
    ...
    try:
        cities_list = cities_service.get_cities(project=project, country_code_iso3=country_code_iso3)
...
def get_city_indicators(city_id: str, admin_level: str):
    try:
        city_indicators = cities_service.get_city_indicators(city_id=city_id, admin_level=admin_level)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"An error occurred: {e}") from e
...
def get_city_geometry(city_id: str, admin_level: str):
    try:
        city_geojson = cities_service.get_city_geometry(city_id=city_id, admin_level=admin_level)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"An error occurred: {e}") from e

    return city_geojson

@router.get("/{city_id}/{admin_level}/geojson/indicators", responses=GET_CITY_GEOMETRY_WITH_INDICATORS_RESPONSES)
...
def get_city_geometry_with_indicators(city_id: str, admin_level: str):
    try:
        city_indicators = cities_service.get_city_geometry_with_indicators(city_id=city_id, admin_level=admin_level)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"An error occurred: {e}") from e
